...
def get_flight_details(flight_id=None, source_city=None, dest_city=None):
    if flight_id:
        if flight_id in data:
            source, dest, price = data[flight_id]
            print(f"Flight ID: {flight_id}")
            print(f"Source: {city[source]}")
            print(f"Destination: {city[dest]}")
            print(f"Price: {price}")
        else:
            print("Flight not found.")
    elif source_city:
        matching_flights = [(fid, source, dest, price) for fid, (source, dest, price) in data.items() if source == source_city]
        if matching_flights:
            print("Flights from", city[source_city])
            for fid, source, dest, price in matching_flights:
                print(f"Flight ID: {fid}")
                print(f"Destination: {city[dest]}")
                print(f"Price: {price}")
        else:
            print("No flights found from", city[source_city])
    elif dest_city:
        matching_flights = [(fid, source, dest, price) for fid, (source, dest, price) in data.items() if dest == dest_city]
        if matching_flights:
            print("Flights to", city[dest_city])
            for fid, source, dest, price in matching_flights:
                print(f"Flight ID: {fid}")
                print(f"Source: {city[source]}")
                print(f"Price: {price}")
        else:
            print("No flights found to", city[dest_city])
    else:
